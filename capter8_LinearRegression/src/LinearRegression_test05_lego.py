...
def useStandRegres():
	...
	print('useStandRegres: %f%+f*年份%+f*部件数量%+f*是否为全新%+f*原价' % (ws[0],ws[1],ws[2],ws[3],ws[4]))	
...
Returns:无
"""
def useCrossValidationRidgeRegres():
	lgX = []
	lgY = []
	setDataCollect(lgX, lgY)
	LinearRegression.crossValidation(lgX, lgY, 10)
...
def usesklearn():
	...
	setDataCollect(lgX, lgY)
	reg.fit(lgX, lgY) 
	print('usesklearn: %f%+f*年份%+f*部件数量%+f*是否为全新%+f*原价' % (reg.intercept_, reg.coef_[0], reg.coef_[1], reg.coef_[2], reg.coef_[3]))	

if __name__ == '__main__':
	usesklearn()
	useStandRegres()
	useCrossValidationRidgeRegres()