    ...
    while (iter_num < maxIter):
        ...
        for i in range(m):
            ...
            if ((labelMat[i]*Ei < -toler) and (alphas[i] < C)) or ((labelMat[i]*Ei > toler) and (alphas[i] > 0)):
                ...
                if eta >= 0: print("eta>=0"); continue #用到半正定矩阵性质： 设A为实对称矩阵，若对于每个非零实向量X，都有X'AX≥0，则称A为半正定矩阵。 假设A=[1,-1,0,...,0],则2A（i,j）-A(i,i)-A(j,j)<=0
                #步骤4：更新alpha_j
...
Returns:
    无
"""
def showClassifer(dataMat, classLabels, alphas, w, b):
    #绘制样本点
    data_plus = []                                  #正样本
    data_minus = []                                 #负样本
