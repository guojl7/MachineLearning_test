...
parser.add_argument(
    ...
    default='E:/MachineLearning_base/DeepLearningModels/src/YAD2K/data/PascalVOC/VOCdevkit')
...
def _main(args):
    ...
    train_boxes = train_group.create_dataset('boxes', shape=(total_train_ids, ), dtype=vlen_int_dt)
    val_boxes = val_group.create_dataset('boxes', shape=(len(val_ids), ), dtype=vlen_int_dt)
    test_boxes = test_group.create_dataset('boxes', shape=(len(test_ids), ), dtype=vlen_int_dt)
    ...
    add_to_dataset(voc_path, '2007_test', test_ids, test_images, test_boxes)
