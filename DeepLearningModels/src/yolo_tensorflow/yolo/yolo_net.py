...
class YOLONet(object):  #定义一个YOLONet类
    ...
    def __init__(self, is_training=True):
        ...
        if is_training:
            self.labels = tf.placeholder(tf.float32, [None, self.cell_size, self.cell_size, 5 + self.num_class]) #label 7x7x(5 + 20)
            self.loss_layer(self.logits, self.labels)
            self.total_loss = tf.losses.get_total_loss()
            tf.summary.scalar('total_loss', self.total_loss)

    def build_network(self,
                      images,        #建立网络（卷积层+池化层+全连接层）
                      num_outputs,   #输入的图像 [None，448,448,3]
                      alpha,
        ...
        with tf.variable_scope(scope):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                                activation_fn=leaky_relu(alpha),                                 #激活函数用的是leaky_rel
                ...
                net = tf.pad(images, np.array([[0, 0], [3, 3], [3, 3], [0, 0]]), name='pad_1')   #为输入图像进行填充，单张图上下左右各用0填充3行/列
                net = slim.conv2d(net, 64, 7, 2, padding='VALID', scope='conv_2')                #input=net; num_outputs=64������ͼ;kernel_size:7X7; strides=2;�����Ѿ�pad�ˣ�����ѡpadding=VALID������ͣ����ͼ���Ե
                ...
                net = slim.fully_connected(net, num_outputs, activation_fn=None, scope='fc_36')              #ȫ���Ӳ㣬���net:7x7x30����
        return net
    ...
    def loss_layer(self, predicts, labels, scope='loss_layer'):
        with tf.variable_scope(scope):
            ...
            #预测的框 batchsize x 7x7x2,每个box四个位置坐标信息
            ...
            #label后（1,2,3,4）位置：目标坐标
            boxes = tf.reshape(labels[..., 1:5], [self.batch_size, self.cell_size, self.cell_size, 1, 4])
            #由于单个cell预测boxes_per_cell个box信息，先对box进行该维度上的拼贴一份相同尺度的；后将坐标尺度归一化到整幅图
            boxes = tf.tile(boxes, [1, 1, 1, self.boxes_per_cell, 1]) / self.image_size
             #label后[5:25]位置：目标类别信息
            classes = labels[..., 5:]

            #将offset维度由7x7x2 reshape成 1x7x7x2
            offset = tf.reshape(tf.constant(self.offset, dtype=tf.float32), [1, self.cell_size, self.cell_size, self.boxes_per_cell])
            #将offset的第一个维度拼贴为batchsize大小，即offset变为：batchsize x 7x7x2
            ...
            iou_predict_truth = self.calc_iou(predict_boxes_tran, boxes)  ##计算IOU的值,[BATCH_SIZE, CELL_SIZE, CELL_SIZE, BOXES_PER_CELL]
            ...
            object_mask = tf.cast((iou_predict_truth >= object_mask), tf.float32) * response  #object_mask:表示有目标 以及 目标与gt的IOU 
            ...
            #对7x7x20每个维度上预测的类别做误差平方求和后，乘以损失函数系数class_scale
