...
from yolo_tensorflow.yolo.yolo_net import YOLONet
...
class Detector(object):
    ...
    #在输入图像img上对检测到的result进行绘制框并标注类别概率信息
    def draw_result(self, img, result):
        for i in range(len(result)): #目标个数遍历绘图
            ...
            cv2.rectangle(img, (x - w, y - h - 20), (x + w, y - h), (125, 125, 125), -1) #显示目标类别和概率值的灰色填充框
    ...
    def detect(self, img):
        ...
        for i in range(len(result)): #检测到目标中心坐标x是448下的坐标，需要变化到原图像尺寸
    ...
    def detect_from_cvmat(self, inputs): #输入的inputs；[1,448,448,3]
        net_output = self.sess.run(self.net.logits, feed_dict={self.net.images: inputs}) #网络回归输出目标
        results = []
    ...
    def interpret_output(self, output):
        probs = np.zeros((self.cell_size, self.cell_size, self.boxes_per_cell, self.num_class)) #所有box (98个)对应每个类别的概率，[7,7,2,20]
        #最后输出时，每个cell只返回一个类别，因此类别概率维度变为[7,7,20]
        class_probs = np.reshape(output[0:self.boundary1], (self.cell_size, self.cell_size, self.num_class)) 
        #输出的[7x7x20:7x7x22]这98个数reshape成[7,7,2]，个人理解是有无目标落在这98个box中
        scales = np.reshape(output[self.boundary1:self.boundary2], (self.cell_size, self.cell_size, self.boxes_per_cell))
        #输出的[7x7x22:]这些数记录的是每个box对应的目标坐标信息，reshape为[7,7,2,4]
        ...
        offset = np.transpose(np.reshape(offset,[self.boxes_per_cell, self.cell_size, self.cell_size]), (1, 2, 0)) #offset；[2,7,7]->[7,7,2]
        ...
        for i in range(self.boxes_per_cell):
            for j in range(self.num_class):
                #某cell中第i个box中含目标的概率*该cell中数据第j个类别概率
                probs[:, :, i, j] = np.multiply(class_probs[:, :, j], scales[:, :, i])

        #若概率大于0.2，filter_mat_probs=1
        filter_mat_probs = np.array(probs >= self.threshold, dtype='bool')
        #过滤掉一个cell中的两个box的其中一个，返回filter_mat_probs中不为0的下标
