...
function usually called by our neural network1 code.
"""
...
def load_data():
    ...
    This is a numpy ndarray with 50,000 entries.  
...
def load_data_wrapper():
    ...
    and ``y`` is the corresponding classification, i.e., the digit values (integers) corresponding to ``x``.

    Obviously, this means we're using slightly different formats for the training data and the validation / test data.  
    These formats turn out to be the most convenient for use in our neural network1 code."""
