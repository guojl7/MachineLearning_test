...
class CrossEntropyCost(object):
    ...
    def fn(a, y):
        """Return the cost associated with an output ``a`` and desired output``y``.  
        Note that np.nan_to_num is used to ensure numericalstability.  
        In particular, if both ``a`` and ``y`` have a 1.0 in the same slot, then the expression (1-y)*np.log(1-a) returns nan. 
...
class Network(object):
    def __init__(self, sizes, cost=CrossEntropyCost):
        """The list ``sizes`` contains the number of neurons in the respective layers of the network1.
        For example, if the list was [2, 3, 1] then it would be a three-layer network1, 
        with the first layer containing 2 neurons,  the second layer 3 neurons, and the third layer 1 neuron.  
        The biases and weights for the network1 are initialized randomly, 
    ...
    def feedforward(self, a):
        """Return the output of the network1 if ``a`` is input."""
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        """Train the neural network1 using mini-batch stochastic gradient descent.
        The ``training_data`` is a list of tuples ``(x, y)`` representing the training inputs and the desired outputs.  
        The other non-optional parameters are self-explanatory, as is the regularization parameter ``lmbda``.  
        ...
        The method returns a tuple containing four lists: the (per-epoch) costs on the evaluation data, 
        the accuracies on the evaluation data, the costs on the training data, and the accuracies on the training data.  
        All values are evaluated at the end of each training epoch.  
        So, for example,  if we train for 30 epochs, then the first element of the tuple will be a 30-element list containing 
        the cost on the evaluation data at the end of each epoch. Note that the lists are empty if the corresponding flag is not set."""
        ...
        for j in xrange(epochs):
            ...
            if monitor_training_accuracy:
                ...
                print "Accuracy on training data: {}%".format((float)(accuracy*100)/n)
            ...
            if monitor_evaluation_accuracy:
                ...
                print "Accuracy on evaluation data: {} %".format((float)(accuracy*100)/n_data)
            print
        return evaluation_cost, evaluation_accuracy, training_cost, training_accuracy

    def update_mini_batch(self, mini_batch, eta, lmbda, n):
        """Update the network1's weights and biases by applying gradient descent using backpropagation to a single mini batch.  
        The ``mini_batch`` is a list of tuples ``(x, y)``, ``eta`` is the learning rate, 
    ...
    def accuracy(self, data, convert=False):
        """Return the number of inputs in ``data`` for which the neural network1 outputs the correct result. 
        The neural network1's output is assumed to be the index of whichever neuron in the final layer has the highest activation.
        ...
        In particular, it flags whether we need to convert between the different representations.  
        ...
        It's done for efficiency reasons -- the program usually evaluates the cost on the training data and the accuracy on other data sets.
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        The flag ``convert`` should be set to False if the data set is the validation or test data (the usual case), 
        and to True if the data set is the training data."""
        cost = 0.0
        for x, y in data:
            a = self.feedforward(x)
            if False == convert: y = vectorized_result(y)
            cost += self.cost.fn(a, y)/len(data)
        cost += 0.5*(lmbda/len(data))*sum(np.linalg.norm(w)**2 for w in self.weights)
        return cost

    def save(self, filename):
        """Save the neural network1 to the file ``filename``."""
...
#### Loading a Network
def load(filename):
    """Load a neural network1 from the file ``filename``.  Returns an instance of Network."""
    f = open(filename, "r")
    data = json.load(f)
    f.close()
