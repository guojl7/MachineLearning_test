...
Chapter 6 of the book 'Neural Networks and Deep Learning'.  
The code essentially duplicates (and parallels) what is in the text, so this is simply a convenience, 
and has not been commentedin detail.  Consult the original text for more details."""
...
def dbl_conv(n=3, epochs=60, activation_fn=sigmoid):
    nets = []
    for j in range(n):
        print "Conv + Conv + FC architecture"
        net = Network([ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), filter_shape=(20, 1, 5, 5), poolsize=(2, 2),activation_fn=activation_fn),
                       ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12), filter_shape=(40, 20, 5, 5), poolsize=(2, 2),activation_fn=activation_fn),
                       FullyConnectedLayer(n_in=40*4*4, n_out=100, activation_fn=activation_fn),
                       SoftmaxLayer(n_in=100, n_out=10)], mini_batch_size)
        net.SGD(training_data, epochs, mini_batch_size, 0.1, validation_data, test_data)
        nets.append(net)
    return nets

"""The following experiment was eventually omitted from the chapter, but I've left it in here, 
since it's an important negative result: basic l2 regularization didn't help much.  
The reason (I believe) is that using convolutional-pooling layers is already a pretty strong regularizer."""
def regularized_dbl_conv(n=3, epochs=60):
    nets = []
    for lmbda in [0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]:
        for j in range(n):
            ...
            net = Network([ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), filter_shape=(20, 1, 5, 5), poolsize=(2, 2)),
                           ...
                           SoftmaxLayer(n_in=100, n_out=10)], mini_batch_size)
            net.SGD(training_data, epochs, mini_batch_size, 0.1, validation_data, test_data, lmbda=lmbda)
            nets.append(net)
    return nets

def dbl_conv_relu(n=3, epochs=60):
    nets = []
    for lmbda in [0.0, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0]:
        for j in range(n):
            ...
            net = Network([ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), filter_shape=(20, 1, 5, 5), poolsize=(2, 2), activation_fn=ReLU),
                           ...
                           SoftmaxLayer(n_in=100, n_out=10)], mini_batch_size)
            net.SGD(training_data, epochs, mini_batch_size, 0.03, validation_data, test_data, lmbda=lmbda)
            nets.append(net)
    return nets

"""Some subsequent functions may make use of the expanded MNIST data.  That can be generated by running expand_mnist.py."""
def expanded_data(n=3, epochs=60, FC_n=100):
    """FC_n is the number of neurons in the fully-connected layer.  We'll try FC_n=100, 300, and 1000."""
    nets = []
    for j in range(n):
        print "Training with expanded data, %s neurons in the FC layer, run num %s" % (FC_n, j)
        net = Network([ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), filter_shape=(20, 1, 5, 5), poolsize=(2, 2), activation_fn=ReLU),
                       ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12), filter_shape=(40, 20, 5, 5), poolsize=(2, 2), activation_fn=ReLU),
                       FullyConnectedLayer(n_in=40*4*4, n_out=FC_n, activation_fn=ReLU),
                       SoftmaxLayer(n_in=FC_n, n_out=10)], mini_batch_size)
        net.SGD(expanded_training_data, epochs, mini_batch_size, 0.03, validation_data, test_data, lmbda=0.1)
        nets.append(net)
    return nets

def expanded_data_double_fc(n=3, epochs=60, FC_n=100):
    """ FC_n is the number of neurons in both fully-connected layers.  We'll try FC_n=100, 300, and 1000."""
    expanded_training_data, _, _ = network3.load_data_shared("../data/mnist_expanded.pkl.gz")
    nets = []
    for j in range(n):
        print "Training with expanded data, %s neurons in two FC layers, run num %s" % (FC_n, j)
        net = Network([ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), filter_shape=(20, 1, 5, 5), poolsize=(2, 2), activation_fn=ReLU),
                       ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12), filter_shape=(40, 20, 5, 5), poolsize=(2, 2), activation_fn=ReLU),
                       FullyConnectedLayer(n_in=40*4*4, n_out=FC_n, activation_fn=ReLU),
                       FullyConnectedLayer(n_in=FC_n, n_out=FC_n, activation_fn=ReLU),
                       SoftmaxLayer(n_in=FC_n, n_out=10)], mini_batch_size)
        net.SGD(expanded_training_data, epochs, mini_batch_size, 0.03, validation_data, test_data, lmbda=0.1)
        nets.append(net)
    return nets

def double_fc_dropout(p0, p1, p2, n=5, epochs=40):
    nets = []
    for j in range(n):
        ...
        net = Network([ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), filter_shape=(20, 1, 5, 5), poolsize=(2, 2), activation_fn=ReLU),
                       ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12), filter_shape=(40, 20, 5, 5), poolsize=(2, 2), activation_fn=ReLU),
                       FullyConnectedLayer(n_in=40*4*4, n_out=1000, activation_fn=ReLU, p_dropout=p0),
                       FullyConnectedLayer(n_in=1000, n_out=1000, activation_fn=ReLU, p_dropout=p1),
                       SoftmaxLayer(n_in=1000, n_out=10, p_dropout=p2)], mini_batch_size)
        net.SGD(expanded_training_data, epochs, mini_batch_size, 0.03, validation_data, test_data)
        nets.append(net)
    return nets

def ensemble(nets): 
    """Takes as input a list of nets, and then computes the accuracy on the test data wheRetn 
    classifications are computed by taking a vote amongst the nets.  
    urns a tuple containing a list of indices for test data which is erroneously classified, 
    and a list of the corresponding erroneous predictions.

    Note that this is a quick-and-dirty kluge: it'd be more reusable (and faster) to define a Theano function taking the vote.  
    But this works. """
...
def run_experiments():
    ...
    Note that the later experiments require access to the expanded training data, which can be generated by running expand_mnist.py."""
    nets = []
    nets.extend(shallow(n=1))
    nets.extend(basic_conv(n=1))
    nets.extend(omit_FC(n=1))
    nets.extend(dbl_conv(n=1, activation_fn=sigmoid))
    # omitted, but still interesting: regularized_dbl_conv()
    nets.extend(dbl_conv_relu(n=1))
    nets.extend(expanded_data(n=1, FC_n=100))
    nets.extend(expanded_data(n=1, FC_n=300))
    nets.extend(expanded_data(n=1, FC_n=1000))
    nets.extend(expanded_data_double_fc(n=1, FC_n=100))  
    nets.extend(expanded_data_double_fc(n=1, FC_n=300))
    nets.extend(expanded_data_double_fc(n=1, FC_n=1000))
    nets.extend(double_fc_dropout(0.5, 0.5, 0.5, n=5))
