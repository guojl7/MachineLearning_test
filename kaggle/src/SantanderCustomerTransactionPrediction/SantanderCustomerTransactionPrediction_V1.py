...
def feature_Engineering5(train_validate_df_org, test_df_org):
    ...
    combined_train_validate_test_X.loc[:,:] = preprocessing.scale(np.array(combined_train_validate_test_X))
    combined_train_validate_test_X['extern'] = np.std(np.array(combined_train_validate_test_X)[:, s], axis=1) + np.mean(np.array(combined_train_validate_test_X)[:, m2], axis=1) - np.mean(np.array(combined_train_validate_test_X)[:, m1], axis=1)
    ...
    train_data_X, validate_data_X, train_data_Y, validate_data_Y = train_test_split(np.array(train_validate_X), np.array(train_validate_Y), test_size=0.1, random_state=7)
    np.savez("train_validate_test_data_extern.npz", train_data_X, validate_data_X, train_data_Y, validate_data_Y, np.array(test_X))
    '''
    
    r = np.load("./train_validate_test_data_extern.npz")
    train_data_X = r["arr_0"]
    validate_data_X = r["arr_1"]
    train_data_Y = r["arr_2"]
    validate_data_Y = r["arr_3"]
...
if __name__ == '__main__':
    train_validate_df_org = pd.read_csv('./data/train.csv')
    test_df_org = pd.read_csv('./data/test.csv')
    ID_code = test_df_org['ID_code']
    
    #特征工程处理
    train_data_X, train_data_Y, validate_data_X, validate_data_Y, test_data_X = feature_Engineering5(train_validate_df_org, test_df_org)
    ...
    pd.DataFrame({"ID_code": ID_code, "target": test_data_Y.reshape(-1)}).to_csv('./data/test_Y.csv', index = False, header = True)
